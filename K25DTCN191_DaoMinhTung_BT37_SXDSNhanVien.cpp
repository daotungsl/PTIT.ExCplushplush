...
void NhanVien::nhap() {
	...
	while(true) {
		...
		if(gioiTinh == "Nam" || gioiTinh == "Nu") break;
	...
	while(true) {
		...
		if(maSoThue.length() == 10 && maSoThue.find_first_not_of("0123456789") == string::npos) break;
	...
	while(true) {
		...
		if(chuanHoaNgay(ngayKyHopDong)) break;
		cout << "Loi: Ngay ky hop dong khong dung dinh dang. Nhap lai: ";
	}
}

void NhanVien::xuat() {
	cout << maNV << " " << hoTen << " " << gioiTinh << " " << ngaySinh << " " << diaChi << " " << maSoThue << " " << ngayKyHopDong << " " << endl;
}
